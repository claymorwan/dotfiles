...
```conf
$blur_layer = dms:(notification-center-modal|workspace-overview|color-picker|clipboard|spotlight|settings|process-list-modal)
layerrule = blur, $blur_layer
...
```
#### Animating the shell with layer rules
```conf
layerrule = animation slide right, dms:(control-center|notification-center-popout|battery)
layerrule = animation slide top, dms:(dash|system-update|process-list-popout|popout)
layerrule = animation slide bottom, dms:dock
layerrule = animation slide left, dms:app-launcher
layerrule = animation slide top, dms:workspace-overview
layerrule = animation fade, dms:(notification-center-modal|color-picker|clipboard|spotlight|settings|process-list-modal)
```
### Performance Tuning
